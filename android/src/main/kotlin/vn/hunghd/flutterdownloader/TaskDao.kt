...
class TaskDao(private val dbHelper: TaskDbHelper) {
    ...
    fun insertOrUpdateNewTask(
        ...
        values.put(
            TaskEntry.COLUMN_SAVE_IN_PUBLIC_STORAGE,
            if (saveInPublicStorage) 1 else 0
        )
        db.beginTransaction()
        try {
    ...
    fun loadTask(taskId: String): DownloadTask? {
        val db = dbHelper.readableDatabase
        val whereClause: String = TaskEntry.COLUMN_NAME_TASK_ID.toString() + " = ?"
    ...
    fun updateTask(taskId: String, status: Int, progress: Int) {
        ...
        try {
            db.update(
                TaskEntry.TABLE_NAME,
                values,
                TaskEntry.COLUMN_NAME_TASK_ID.toString() + " = ?",
                arrayOf(taskId)
            )
            db.setTransactionSuccessful()
        } catch (e: Exception) {
    ...
    private fun parseCursor(cursor: Cursor): DownloadTask {
        ...
        val status =
            cursor.getInt(cursor.getColumnIndexOrThrow(TaskEntry.COLUMN_NAME_STATUS))
        val progress =
            cursor.getInt(cursor.getColumnIndexOrThrow(TaskEntry.COLUMN_NAME_PROGRESS))
        val url =
            cursor.getString(cursor.getColumnIndexOrThrow(TaskEntry.COLUMN_NAME_URL))
        val filename =
        ...
        val resumable =
            cursor.getShort(cursor.getColumnIndexOrThrow(TaskEntry.COLUMN_NAME_RESUMABLE))
                .toInt()
